---
title: "Single-cell Toolkit Lab (10X 2.7K PBMCs)"
...
```{r}
counts_matrix_filename = "/data/data/pbmcs/pbmc3k.counts.matrix.gz"
...
pbmc=JackStraw(pbmc, num.pc = 20, num.replicate = 100, prop.freq = 0.01, display.progress = TRUE, do.par = TRUE, num.cores = 2, maxit = 1000)
...
Given the sparse measurements recovered by scRNAseq a popular approach involves aggregating (possibly weak) signals across groups of cells. One popular approach to accomplishing this is by gene set enrichment methods (GSEA).

A simple approach which is not unique to single cell analysis is to calculate a global rank order statistics and search for gene-sets showing relative enrichment at the top of the list. Applying this at the scale of droplet based scRNAseq datasets is sometimes challanging with older tools.

### Calculating an informative order statistic  

The sample size of these experiments allows for effective use of a particular class statistics - area under the ROC or precision-recall curves.

```{r}

normExp = pbmc@scale.data
labelValue = pbmc@ident == "CD8 T cells"
table(labelValue)

aucfun <- function(x) {auc(evalmod(scores = x,labels = labelValue,posclass=T))$aucs}
outAUCtable <- pbapply(normExp,1,aucfun)

outDiff = data.frame(geneID=rownames(normExp),AUC=outAUCtable[1,],PR=outAUCtable[2,])
outDiff = outDiff[order(outDiff$PR,decreasing = T),]
head(outDiff,20)
...
There are many sources for gene-set information. On popular source is msigdb which aggegates information from multiple other sources. Here we will only use the GO portion of msigDB. 

```{r}

msigdbGO = gmtPathways("/data/pathway/c5.all.v6.1.symbols.gmt")
head(msigdbGO,3)
...
fGSEA is a well optimized method for running rank order GSEA (ssGSEA). It saves considerable time by precalculating the permutations once for all gene sets. It likely helps that it is implemented well in C++ code. 

```{r}
# PR 
orderStat = outAUCtable[2,]
names(orderStat) = rownames(normExp)

fgseaRes <- fgsea(pathways = msigdbGO, 
                  stats = orderStat,
                  minSize=15,
                  maxSize=500,
                  nperm=100000)
...
### Visalizing the enrichment and the "leading edge" 

We can examine the enrichment in our ranked list. The "leading edge", refers to the set of genes observed at the point of maximum enrichment. 

```{r}
gsLabel = "GO_REGULATION_OF_LEUKOCYTE_MEDIATED_CYTOTOXICITY"
plotEnrichment(msigdbGO[[gsLabel]],
               orderStat) + labs(title=gsLabel)
```
...
plotGseaTable(msigdbGO[topPathways], orderStat, fgseaRes, 
              gseaParam = 0.5)

```

Note that in scRNAseq the negative class genes are often less interesting, due at least in part to the sparsity in measurements.   

## Calculating the enrichment of a particular gene-set ("or signature") in individual cells 

Often we wish to have an enrichment score for individual cells. A simple and curde approach is to calculate the mean/median over the expression of genes in a sets in every cell. This tends to work for large gene-sets of highly expressed genes. Another approach is to calculate the area under the gene set recovery curve. 

Here we will use hematopetic signatures collected in the xCell paper. 
...
load('/data/pathway/xCell.data.rda')
geneSets = xCell.data$signatures

cleanNames = sub('%.*','',names(geneSets))
qList = read.table('/data/pathway/queryList.lst',header=F,sep="\t",stringsAsFactors = F)$V1

geneSets = geneSets[cleanNames %in% qList]
...
exprMatrix = pbmc@data
cells_rankings <- AUCell_buildRankings(exprMatrix, nCores=1, plotStats=TRUE)

```

We next calculate the area under the gene set recovery curve for every cell
...
### Calcualte cell level thresholds 
...
### Visualize cell type "AUC" scores in a tSNE

```{r}

expAUC = getAUC(cells_AUC)
selType = "CD4+ T-cells%HPCA%1.txt"
cd4_Tcell = expAUC[selType,]

pbmc@meta.data$cd4_score = cd4_Tcell

### Plotting a prior known genes
FeaturePlot(object = pbmc, features.plot = 'cd4_score', reduction.use = "tsne")

```

### A binarized statistic

AUCcell attempts to also estimate a binary statistic 

```{r}

minScore = cells_assignment[[selType]]$aucThr$thresholds["minimumDens",1]

### Plotting a prior known genes
FeaturePlot(object = pbmc, features.plot = 'cd4_score', reduction.use = "tsne",min.cutoff = minScore)

table(pbmc@ident,cd4_Tcell>minScore)
...
In cases where the question of interest has to do with a continuum of transcriptional changes like during development or differentiation (e.g. hematopoiesis), we may not expect to see discrete cell-types which cluster out. Instead, we expect to see a gradation. Diffusion maps offer a low dimensional embedding that supports continuous state transitions as one approach to map transcriptional trajectories. 

While the pbmc data does not represent a differentiating system, we will practice `Seurat`'s diffusion map function. 
